...
The full vanilla corpus contains 252 expression records in 91 modifiers across 36 files. Of those, 248 are variable assignments and all use assignment, numeric literals, names, unary signs, parentheses, arithmetic `+`, `-`, `*`, `/`, comparisons, `&&`, `||`, `!`, and the `clamp(value, low, high)` and `cond(test, whenTrue, whenFalse)` functions. All 248 parse and evaluate from their files' starting values. The remaining four use `EventName if (predicate)` and have no assignment variable or event index in their native records, so they are event predicates outside this variable-assignment scope. Dogmeat supplies 17 of the assignment records and remains the native runtime regression fixture. This is deliberately not a general Havok scripting implementation.
